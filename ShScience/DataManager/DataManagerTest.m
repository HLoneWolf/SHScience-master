...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        ...
        {
            [user generateMAC];
            [DataManager registWithUser:user callback:^(BOOL registed, BOOL success) {
                //
            }];
        }
            break;
        ...
        case 2:
        {
            ...
            [DataManager updateUserInfo:user callback:^(BOOL update, BOOL success) {
                //
            }];
        }
            break;
...
//                                    if (success) {
//                                        NSLog(@"user: %@", user.sid);
//                                        [DataManager loadPreorderWithScheduleItem:schItem user:user num:@1 playDate:[NSDate date] callback:^(NSString *code, BOOL success) {
//                                            NSLog(@"preorder: %@", code);
//                                        }];
//                                    }
//                                }];
...
//            }];
        ...
        case 17:
        {
            ...
            DataRequest *request = [DataManager loadPointInfoWithUser:user floor:1 callback:^(NSArray *infos, BOOL success) {
                //
                NSLog(@"infos: %@", infos);
            } ];
        ...
        case 21:
        {
            InfoItem *infoItem = [[InfoItem alloc] init];
            infoItem.sid = @60;
            [DataManager postPraiseWithInfoItem:infoItem callback:^(NSNumber *praiseNum, BOOL success) {
                NSLog(@"praiseNumber: %i", praiseNum.intValue);
            }];
        }
            break;
...
//            }];
        ...
        case 39:
        {
            DataRequest *request = [DataManager loadInterestGroupWithPage:0 pagesize:20 callback:^(PageItem *pageItem, BOOL success) {
        ...
        case 40:
        {
            InterestGroupItem *groupItem = [[InterestGroupItem alloc] init];
            groupItem.sid = @3;
            user.sid = @8;
            DataRequest *request = [DataManager joinInterestGroup:groupItem user:user callback:^(BOOL joined, BOOL success) {
                //
                if (success) {
                    NSLog(@"加入成功: %@", joined?@"YES":@"NO");
                }
            }];
        ...
        case 41:
        {
            ...
            InterestGroupItem *groupItem = [[InterestGroupItem alloc] init];
            groupItem.sid = @3;
            DataRequest *request = [DataManager loadInterestGroupIsJoinedWithUser:user group:groupItem callback:^(BOOL joined, BOOL success) {
                if (success)
                {
                    NSLog(@"是否小组成员: %@", joined?@"YES":@"NO");
                }
            }];
        ...
        case 42:
        {
            ...
            user.sid = @8;
//            DataRequest *request = [DataManager postInterestWithPost:postItem user:user callback:^(BOOL posted, BOOL success) {
//                if (success) {
//                    NSLog(@"发帖: %@", posted?@"YES":@"NO");
//                }
//            }];
//            
//            [request startAsynchronous];
        ...
        case 43:
        {
            PostItem *postItem = [[PostItem alloc] init];
            postItem.sid = @1;
            postItem.content = @"ContentReply";
            user.sid= @8;
            postItem.replyContent = @"回复";
            DataRequest *request = [DataManager replyInterestWithPost:postItem user:user callback:^(BOOL posted, BOOL success){
                if (success) {
                    NSLog(@"回帖: %@", posted?@"YES":@"NO");
                }
            }];
        ...
        case 44:
        {
            ...
            InterestGroupItem *groupItem = [[InterestGroupItem alloc] init];
            groupItem.sid = @3;
            DataRequest *request = [DataManager loadPostsWithInterestGroup:groupItem catalog:PostCatalogAll page:1 pagesize:30 callback:^(PageItem *pageItem, BOOL success) {
                if (success)
                {
                    NSLog(@"兴趣小组的帖子: %@", pageItem);
                }
            }];
            [request startAsynchronous];
        ...
        case 45:
        {
            ...
            PostItem *postItem = [[PostItem alloc] init];
            postItem.sid = @1;
//            DataRequest *request = [DataManager loadReplyWithPost:postItem callback:^(PageItem *pageItem, BOOL success) {
//                if (success)
//                {
//                    NSLog(@"pageItem: %@", pageItem);
//
