...
{
    UIFont *font = [UIFont fontWithName:@"HelveticaNeue" size:16.f];
    
    CGSize screenSize = [self screenSize];
    CGSize textSize = [text sizeWithFont:font constrainedToSize:CGSizeMake(screenSize.width - 10.f*4.f, 1000.f) lineBreakMode:NSLineBreakByWordWrapping];
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withViewArray:(NSArray *)viewArray
{
    ...
    for (UIView *view in viewArray) {
        ...
        float padding = (i == viewArray.count-1) ? 0 : 10.f;
    ...
    for (UIView *view in viewArray) {
        ...
        if (NO && i != viewArray.count-1) {
            CGRect dividerRect = CGRectMake(view.frame.origin.x, floorf(view.frame.origin.y + view.frame.size.height + 10.f*0.5f), view.frame.size.width, 0.5f);
            
            [((NSMutableArray *)dividerRects) addObject:[NSValue valueWithCGRect:dividerRect]];
        }
        
        //Only add padding below the view if it's not the last item
        float padding = (i == viewArray.count-1) ? 0.f : 10.f;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withTitle:(NSString *)title withViewArray:(NSArray *)viewArray
{
    ...
    titleLabel.textColor = [UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:1];
    ...
    float titleHeightOffset = (titleSize.height > 0.f ? 10.f : 0.f);
    
    float totalHeight = titleSize.height + titleHeightOffset + 10.f;
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withStringArray:(NSArray *)stringArray
{
    ...
    for (NSString *string in stringArray) {
        ...
        textButton.titleLabel.textColor = [UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:1];
        [textButton setTitle:string forState:UIControlStateNormal];
        textButton.layer.cornerRadius = 4.f;
        [textButton setTitleColor:[UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:1] forState:UIControlStateNormal];
...
- (NSMutableArray*) makeTempViewsWithStrings:(NSArray *)stringArray andImages:(NSArray *)imageArray
{
    ...
    for (int i = 0; i < stringArray.count; i++) {
        ...
        float containerHeight = label.frame.size.height + 3.f + 3.f + imageView.frame.size.height;
        ...
        imageView.frame = CGRectMake(floorf(containerWidth*0.5f - imageView.frame.size.width*0.5f), 3.f, imageView.frame.size.width, imageView.frame.size.height);
        label.frame = CGRectMake(floorf(containerWidth*0.5f - label.frame.size.width*0.5f), imageView.frame.size.height + 3.f + 3.f, label.frame.size.width, label.frame.size.height);
...
-(void)setupLayout:(CGPoint)point inView:(UIView*)view
{
    ...
    if (arrowPoint.x + 12.f > topViewBounds.size.width - 10.f - 4.f - 5.f) {//Too far to the right
        arrowPoint.x = topViewBounds.size.width - 10.f - 4.f - 5.f - 12.f;
        //NSLog(@"Correcting Arrow Point because it's too far to the right");
    } else if (arrowPoint.x - 12.f < 10.f + 4.f + 5.f) {//Too far to the left
        arrowPoint.x = 10.f + 12.f + 4.f + 5.f;
    ...
    if (xOrigin < CGRectGetMinX(topViewBounds) + 10.f) {
        xOrigin = CGRectGetMinX(topViewBounds) + 10.f;
    } else if (xOrigin + boxWidth > CGRectGetMaxX(topViewBounds) - 10.f) {
        //Check to see if the positioning puts the box out of the window towards the left
        xOrigin = CGRectGetMaxX(topViewBounds) - 10.f - boxWidth;
    }

    float arrowHeight = 12.f;
...
- (void)drawRect:(CGRect)rect
{
    ...
    float radius = 4.f; //Radius of the curvature.
    
    float cpOffset = 1.8f; //Control Point Offset.  Modifies how "curved" the corners are.
    ...
    if (!above) {
        [popoverPath addLineToPoint:CGPointMake(arrowPoint.x - 12.f, yMin)];//left side
        [popoverPath addCurveToPoint:arrowPoint controlPoint1:CGPointMake(arrowPoint.x - 12.f + 6.f, yMin) controlPoint2:arrowPoint];//actual arrow point
        [popoverPath addCurveToPoint:CGPointMake(arrowPoint.x + 12.f, yMin) controlPoint1:arrowPoint controlPoint2:CGPointMake(arrowPoint.x + 12.f - 6.f, yMin)];//right side
    }
    ...
    if (above) {
        [popoverPath addLineToPoint:CGPointMake(arrowPoint.x + 12.f, yMax)];//right side
        [popoverPath addCurveToPoint:arrowPoint controlPoint1:CGPointMake(arrowPoint.x + 12.f - 6.f, yMax) controlPoint2:arrowPoint];//arrow point
        [popoverPath addCurveToPoint:CGPointMake(arrowPoint.x - 12.f, yMax) controlPoint1:arrowPoint controlPoint2:CGPointMake(arrowPoint.x - 12.f + 6.f, yMax)];
    }
    ...
    CGFloat shadowBlurRadius = 3.f;
    
    //// Gradient Declarations
//    NSArray* gradientColors = [NSArray arrayWithObjects:
//                               (id)[UIColor colorWithRed:1.f green:1.f blue:1.f alpha:0.95f].CGColor,
//                               (id)[UIColor colorWithRed:0.98f green:0.98f blue:0.98f alpha:0.95f].CGColor, nil];
    //dsy change 不要颜色渐变
    NSArray* gradientColors = [NSArray arrayWithObjects:
    ...
    float bottomOffset = (above ? 12.f : 0.f);
    float topOffset = (!above ? 12.f : 0.f);
    ...
    if (YES) {
        ...
        if (titleView != nil) {
            titleBGHeight = 10.f*2.f + titleView.frame.size.height;
        }
        ...
        if (titleBGHeight > 0.f) {
            ...
            //In this case, the arrow is located between LT2 and RT1
            if (!above) {
                [titleBGPath addLineToPoint:CGPointMake(arrowPoint.x - 12.f, yMin)];//left side
                [titleBGPath addCurveToPoint:arrowPoint controlPoint1:CGPointMake(arrowPoint.x - 12.f + 6.f, yMin) controlPoint2:arrowPoint];//actual arrow point
                [titleBGPath addCurveToPoint:CGPointMake(arrowPoint.x + 12.f, yMin) controlPoint1:arrowPoint controlPoint2:CGPointMake(arrowPoint.x + 12.f - 6.f, yMin)];//right side
            }
            ...
            NSArray* gradientColors = [NSArray arrayWithObjects:
                                       (id) [UIColor colorWithRed:1.f green:1.f blue:1.f alpha:0.95f].CGColor,
                                       (id)[UIColor colorWithRed:0.93f green:0.93f blue:0.93f alpha:0.95f].CGColor, nil];
            CGFloat gradientLocations[] = {0, 1};
            CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFTYPECAST(CFArrayRef)gradientColors), gradientLocations);
        ...
        if (NO && showDividerRects) {
            if (dividerRects && dividerRects.count > 0) {
                for (NSValue *value in dividerRects) {
                    ...
                    [[UIColor colorWithRed:0.329 green:0.341 blue:0.353 alpha:0.15f] setFill];
                    [dividerPath fill];
                }
            }
        }
