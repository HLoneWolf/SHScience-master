...
- (id)initWithVerticalFrame:(CGRect)frame andTabItems:(NSArray *)tabItems {
    _verticalFlag = @"Y";
    self = [super initWithFrame:frame];
    if (self) {
        self.tabItems = tabItems;
...
//dsy vertical suppport
- (CGFloat)tabVerticalItemWidth {
    return self.frame.size.width;
}

- (CGFloat)tabVerticalItemHeight {
    CGFloat restrictedHeight = self.frame.size.height - self.horizontalInsets.left - self.horizontalInsets.right;
    return self.tabItems.count > 0 ? restrictedHeight/self.tabItems.count : restrictedHeight;
}
...
//dsy
- (CGRect)frameForVerticalTab:(RKTabItem *)tabItem {
    CGFloat width  = [self tabVerticalItemWidth];
    CGFloat height = [self tabVerticalItemHeight];
    //CGFloat x = self.horizontalInsets.left + [self indexOfTab:tabItem] * width;
    CGFloat y = [self indexOfTab:tabItem] * height;
...
- (UIControl *)tabForItem:(RKTabItem *)tabItem {
    ...
    if (tab) {
        ...
        //dsy support vertical
        if([@"Y" isEqualToString:_verticalFlag]){
            tab = [[UIControl alloc] initWithFrame:[self frameForVerticalTab:tabItem]];
        }
        else{
